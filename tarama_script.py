...
print("TEFAS Crawler fonksiyonları hazır.")
        ...
        while retries < max_retries and not success:
            try:
                if current_start_date_chunk <= current_end_date_chunk:
                    chunk_data_fetched = get_funds(start=current_start_date_chunk.strftime("%Y-%m-%d"), end=current_end_date_chunk.strftime("%Y-%m-%d"), fund_code=fon_kodu, columns=F_COLS)
...
def apply_cell_format_request(worksheet_id, row_index, num_columns, is_highlight):
    ...
    return {"repeatCell": {"range": {"sheetId": worksheet_id, "startRowIndex": row_index, "endRowIndex": row_index + 1, "startColumnIndex": 0, "endColumnIndex": num_columns}, "cell": {"userEnteredFormat": {"textFormat": text_format}}, "fields": "userEnteredFormat.textFormat.foregroundColor,userEnteredFormat.textFormat.bold"}}
...
def run_weekly_scan_to_gsheets(num_weeks: int, gc):
    ...
    fon_args_list = [(fon_kodu, genel_veri_cekme_baslangic_tarihi, today, 30, 3, 5) for fon_kodu in all_fon_data_df['Fon Kodu'].unique()]
    MAX_WORKERS = 10
    weekly_results_dict = {}
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        progress_bar = tqdm(concurrent.futures.as_completed(future_to_fon), total=total_fon_count, desc="Fonlar Taranıyor (Haftalık)")
        for future in progress_bar:
            ...
            try:
                ...
                if fon_kodu_completed not in all_fon_data_df['Fon Kodu'].values: fon_adi = "Bilinmiyor"
                ...
                if len(valid_changes) == num_weeks and num_weeks >= 2:
                    if all(valid_changes[j] > valid_changes[j+1] for j in range(num_weeks - 1)): is_desired_trend = True
    ...
    try:
        ...
        worksheet.clear()
        if not results_df.empty:
            worksheet.update(values=[results_df.columns.values.tolist()] + results_df.values.tolist(), value_input_option='USER_ENTERED')
            format_requests = []
            ...
            if format_requests:
                ...
                print("İstenen trende (H1>H2>...) uyan hiçbir fon bulunamadı.")
            body_resize = {"requests": [{"autoResizeDimensions": {"dimensions": {"sheetId": worksheet.id, "dimension": "COLUMNS", "startIndex": 0, "endIndex": len(results_df.columns)}}}]}
            spreadsheet.batch_update(body_resize)
        else:
            print("Google Sheets'e yazılacak veri bulunmuyor.")
        end_time_main = time.time()
        print("\n" + "="*50 + f"\nHAFTALIK TARAMA BAŞARIYLA TAMAMLANDI! ({datetime.now(TIMEZONE).strftime('%d.%m.%Y %H:%M:%S')})\n" + f"Toplam süre: {((end_time_main - start_time_main) / 60):.2f} dakika\n" + "="*50)
    except Exception as e:
...
def run_scan_to_gsheets(scan_date: date, gc, attempt=1):
    ...
    fon_args_list = [(fon_kodu, genel_veri_cekme_baslangic_tarihi, scan_date, 30, 3, 5) for fon_kodu in all_fon_data_df['Fon Kodu'].unique()]
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for future in progress_bar:
            ...
            try:
                _, fund_history = future.result()
                fiyat_son, degisimler = np.nan, {p: np.nan for p in ['Günlük %', 'Haftalık %', '2 Haftalık %', 'Aylık %', '3 Aylık %', '6 Aylık %', '1 Yıllık %', 'YB %']}
                if fund_history is not None and not fund_history.empty:
                    ...
                    if not pd.isna(fiyat_son):
                        ...
                        periods_other = {'Haftalık %': timedelta(weeks=1), '2 Haftalık %': timedelta(weeks=2), 'Aylık %': relativedelta(months=1), '3 Aylık %': relativedelta(months=3), '6 Aylık %': relativedelta(months=6), '1 Yıllık %': relativedelta(years=1)}
                ...
                fon_adi = all_fon_data_df.loc[all_fon_data_df['Fon Kodu'] == fon_kodu_completed, 'Fon Adı'].iloc[0]
                if fon_kodu_completed not in all_fon_data_df['Fon Kodu'].values: fon_adi = "Bilinmiyor"
                result_row = {'Fon Kodu': fon_kodu_completed, 'Fon Adı': fon_adi, 'Bitiş Tarihi': scan_date.strftime("%d.%m.%Y"), 'Fiyat': fiyat_son}
    ...
    column_order = ['Fon Kodu', 'Fon Adı', 'Bitiş Tarihi', 'Fiyat', 'Günlük %', 'Haftalık %', '2 Haftalık %', 'Aylık %', '3 Aylık %', '6 Aylık %', '1 Yıllık %', 'YB %']
    ...
    print(f"Fiyat verisi eksik olan fon sayısı: {missing_price_count}")
    try:
        spreadsheet = gc.open_by_key(SHEET_ID)
        try:
        ...
        if not results_df_tekil.empty:
            ...
            print("Google Sheets'e yazılacak veri bulunmuyor (Tekil Tarama).")
        end_time_main_tekil = time.time()
        print("\n" + "="*50 + f"\nTEKİL TARAMA BAŞARIYLA TAMAMLANDI! (Deneme: {attempt}, {datetime.now(TIMEZONE).strftime('%d.%m.%Y %H:%M:%S')})\n" + f"Toplam süre: {((end_time_main_tekil - start_time_main) / 60):.2f} dakika\n" + "="*50)
...
if __name__ == "__main__":
    ...
    if missing_price_count is not None and missing_price_count >= 5:
        print(f"\nEksik fiyat verisi olan fon sayısı ({missing_price_count}) eşik değerin (5) üzerinde.")
        ...
        print(f"İkinci tarama sonrası eksik fiyat verisi olan fon sayısı: {missing_price_count}")
    else:
