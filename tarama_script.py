...
try:
    # `tefas-crawler` kütüphanesinin güncel API'si
    from tefas_crawler import get_funds
except ImportError as e:
    print(
        f"❌ Hata: tefas-crawler kütüphanesi yüklenemedi veya yanlış sürümde: {e}")
    raise
...
def load_takasbank_fund_list():
    ...
    try:
        ...
        df_data['Fon Kodu'] = df_data['Fon Kodu'].astype(
...
def get_first_price_looking_back(df_fund_history, end_date: date, max_lookback_days: int = 6):
    if df_fund_history is None or df_fund_history.empty:
        return np.nan
    start_lookback_date = end_date - timedelta(days=max_lookback_days)
    end_lookback_date = end_date - timedelta(days=1)
    relevant_history = df_fund_history[(df_fund_history['date'] >= start_lookback_date) & (
        df_fund_history['date'] <= end_lookback_date)]
    if not relevant_history.empty:
        return relevant_history['price'].iloc[0]
    return np.nan
...
def get_price_at_date_or_next_available(df_fund_history, target_date: date, max_lookforward_days: int = 5):
    ...
    first_available = df_fund_history[(df_fund_history['date'] >= target_date) & (
        df_fund_history['date'] <= future_limit_date)].sort_values(by='date', ascending=True)
    if not first_available.empty:
        return first_available['price'].iloc[0]
    return np.nan


def calculate_change(current_price, past_price):
    if pd.isna(current_price) or pd.isna(past_price) or past_price is None or current_price is None:
        return np.nan
    try:
        current_price_float, past_price_float = float(
            current_price), float(past_price)
...
def fetch_data_for_fund_parallel(args):
    ...
    while current_start_date_chunk <= end_date_overall:
        current_end_date_chunk = min(
            current_start_date_chunk + timedelta(days=chunk_days - 1), end_date_overall)
        ...
        while retries < max_retries and not success:
            try:
                ...
                if not chunk_data_fetched.empty:
                    all_fon_data = pd.concat(
                        [all_fon_data, chunk_data_fetched], ignore_index=True)
                success = True
            except Exception as e:
                print(
                    f"Hata fon çekme ({fon_kodu}, {current_start_date_chunk}-{current_end_date_chunk}): {e}")
                retries += 1
    ...
    if not all_fon_data.empty:
        all_fon_data.drop_duplicates(
            subset=['date', 'price'], keep='first', inplace=True)
        if 'date' in all_fon_data.columns:
            all_fon_data['date'] = pd.to_datetime(
                all_fon_data['date'], errors='coerce').dt.date
...
def apply_cell_format_request(worksheet_id, row_index, num_columns, is_highlight):
    if is_highlight:
        text_format = {"foregroundColor": {
            "red": 1.0, "green": 0.0, "blue": 0.0}, "bold": True}
    else:
        text_format = {"foregroundColor": {
            "red": 0.0, "green": 0.0, "blue": 0.0}, "bold": False}
...
def run_weekly_scan_to_gsheets(num_weeks: int, gc):
    ...
    genel_veri_cekme_baslangic_tarihi = today - \
        timedelta(days=(num_weeks * 7) + 21)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        future_to_fon = {executor.submit(
            fetch_data_for_fund_parallel, args): args[0] for args in fon_args_list}
        ...
        for future in progress_bar:
            ...
            try:
                ...
                fon_adi = all_fon_data_df.loc[all_fon_data_df['Fon Kodu']
                                              == fon_kodu_completed, 'Fon Adı'].iloc[0]
                if fon_kodu_completed not in all_fon_data_df['Fon Kodu'].values:
                    fon_adi = "Bilinmiyor"

                current_fon_data = {
                    'Fon Kodu': fon_kodu_completed, 'Fon Adı': fon_adi}
                ...
                for i in range(num_weeks):
                    current_week_start_date_cal = current_week_end_date_cal - \
                        timedelta(days=7)
                    price_end = get_price_on_or_before(
                        fund_history, current_week_end_date_cal)
                    price_start = get_price_on_or_before(
                        fund_history, current_week_start_date_cal)
                ...
                current_fon_data['Değerlendirme'] = calculate_change(
                    first_week_end_price, last_week_start_price)
                is_desired_trend = False
                valid_changes = [
                    chg for chg in weekly_changes_list if not pd.isna(chg)]
                ...
                current_fon_data['_DEBUG_WeeklyChanges_RAW'] = "'" + \
                    str([f"{x:.2f}" if not pd.isna(x) else "NaN" for x in weekly_changes_list])
                current_fon_data['_DEBUG_IsDesiredTrend'] = bool(
                    is_desired_trend)
    ...
    final_view_columns = base_cols + \
        first_fund_calculated_columns + ['Değerlendirme'] + debug_cols
    all_df_columns = final_view_columns + ['is_desired_trend']
    existing_cols_for_df = [
        col for col in all_df_columns if col in results_df.columns]

    if not results_df.empty:
        results_df = results_df[existing_cols_for_df]
        results_df.sort_values(
            by='Değerlendirme', ascending=False, na_position='last', inplace=True)
    else:
        results_df = pd.DataFrame(columns=existing_cols_for_df)

    for col in results_df.columns:
        if results_df[col].dtype == 'float64':
            results_df[col] = results_df[col].replace([np.inf, -np.inf], np.nan).astype(
                object).where(pd.notna(results_df[col]), None)
        elif results_df[col].dtype == 'object' and col not in ['is_desired_trend', '_DEBUG_IsDesiredTrend', '_DEBUG_WeeklyChanges_RAW']:
            results_df[col] = results_df[col].apply(lambda x: None if (isinstance(
                x, str) and (x.lower() in ['nan', 'nat'])) or pd.isna(x) else x)
    ...
    print(
        f"🔄 Sonuçlar Google Sheets'teki '{WORKSHEET_NAME_WEEKLY}' sayfasına yazılıyor...")
    ...
    try:
        ...
        try:
            ...
            worksheet = spreadsheet.add_worksheet(
                title=WORKSHEET_NAME_WEEKLY, rows="1000", cols=max(100, len(final_view_columns) + 5))
        worksheet.clear()
        ...
        if not results_df.empty:
            ...
            for idx, row in results_df.reset_index(drop=True).iterrows():
                if row.get('is_desired_trend', False):
                    format_requests.append(apply_cell_format_request(
                        worksheet.id, idx + 1, len(results_df.columns), True))

            if format_requests:
                spreadsheet.batch_update({"requests": format_requests})
                print(
                    f"✅ {len(format_requests)} satır, istenen trende uyduğu için işaretlendi.")
            else:
...
def run_scan_to_gsheets(scan_date: date, gc, attempt=1):
    ...
    all_results = []
    genel_veri_cekme_baslangic_tarihi = scan_date - \
        ...
        for future in progress_bar:
            ...
            try:
                ...
                if fund_history is not None and not fund_history.empty:
                    fiyat_son = get_price_on_or_before(
                        fund_history, scan_date)

                    if not pd.isna(fiyat_son):
                        fiyat_onceki_gun = get_first_price_looking_back(
                            fund_history, scan_date, max_lookback_days=6)
                        degisimler['Günlük %'] = calculate_change(
                            fiyat_son, fiyat_onceki_gun)
                        ...
                        for name, period_delta in periods_other.items():
                            ...
                            past_price = get_price_at_date_or_next_available(
                                fund_history, past_target_date, max_lookforward_days=5)
                            degisimler[name] = calculate_change(
                                fiyat_son, past_price)

                        target_yb_start_date = date(scan_date.year, 1, 1)
                        fiyat_yb_once = get_price_at_date_or_next_available(
                            fund_history, target_yb_start_date, max_lookforward_days=5)
                        degisimler['YB %'] = calculate_change(
                            fiyat_son, fiyat_yb_once)
    ...
    existing_cols_tekil = [
        col for col in column_order if col in results_df_tekil.columns]

    if not results_df_tekil.empty:
        results_df_tekil = results_df_tekil[existing_cols_tekil].sort_values(
            by='YB %', ascending=False, na_position='last')
    else:
    ...
    for col in results_df_tekil.columns:
        if results_df_tekil[col].dtype == 'float64':
            results_df_tekil[col] = results_df_tekil[col].replace([np.inf, -np.inf], np.nan).astype(
                object).where(pd.notna(results_df_tekil[col]), None)
        elif results_df_tekil[col].dtype == 'object':
            results_df_tekil[col] = results_df_tekil[col].apply(lambda x: None if (isinstance(
                x, str) and (x.lower() in ['nan', 'nat'])) or pd.isna(x) else x)
    ...
    try:
        ...
        try:
            ...
            worksheet_tekil = spreadsheet.add_worksheet(
                title=WORKSHEET_NAME_MANUAL, rows="1000", cols=max(100, len(existing_cols_tekil) + 5))
        worksheet_tekil.clear()

        if not results_df_tekil.empty:
            data_to_upload_tekil = [
                results_df_tekil.columns.values.tolist()] + results_df_tekil.values.tolist()
            worksheet_tekil.update(
                values=data_to_upload_tekil, range_name='A1')
            ...
            print(
                "ℹ️ Google Sheets'e yazılacak veri bulunmuyor (Tekil Tarama).")
...
if __name__ == "__main__":
    ...
    if not gc_auth:
        print(
            "❌ Google Sheets yetkilendirmesi başarısız olduğu için işlem iptal edildi.")
        sys.exit(1)

    today_in_istanbul = datetime.now(TIMEZONE).date()
    print(
        f"Bugünün tarihi (İstanbul Saati): {today_in_istanbul.strftime('%d.%m.%Y')}")

    # --- BİRİNCİ TARAMA ---
    print("\n=== TEKİL TARAMA BAŞLIYOR (1. DENEME) ===")
    results_df, missing_price_count = run_scan_to_gsheets(
        today_in_istanbul, gc_auth, attempt=1)

    # --- YENİDEN TARAMA KONTROLÜ ---
    # Eğer ilk taramada 5 veya daha fazla fonun fiyat verisi eksikse, 20 dakika bekle ve tekrar tara.
    if missing_price_count is not None and missing_price_count >= 5:
        print(
            f"\nℹ️ Eksik fiyat verisi olan fon sayısı ({missing_price_count}) eşik değerin (5) üzerinde.")
        print("... 20 dakika bekleniyor ve tarama yeniden başlatılacak ...")
        time.sleep(1200)  # 20 dakika bekle
        print("\n=== TEKİL TARAMA TEKRAR BAŞLIYOR (2. DENEME) ===")
        results_df, missing_price_count = run_scan_to_gsheets(
            today_in_istanbul, gc_auth, attempt=2)
        print(
            f"ℹ️ İkinci tarama sonrası eksik fiyat verisi olan fon sayısı: {missing_price_count}")
    else:
        print(
            f"\n✅ Eksik fiyat sayısı ({missing_price_count}) eşik değerin altında. Yeniden tarama yapılmayacak.")

    # --- HAFTALIK TARAMA ---
    # Haftalık tarama her durumda çalışır.
